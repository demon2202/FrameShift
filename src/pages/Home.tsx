...
export const Home: React.FC = () => {
  ...
  // 2. Computed Values
  const feed = useMemo(() => user ? getFeed() : [], [user, getFeed]);
      ...
      <main className="max-w-[1400px] mx-auto px-4 md:px-8 pb-20 relative z-10">
        <div className="w-full space-y-16">
            ...
            <div className="w-full space-y-16">
                ...
                {/* 1. Enhanced Stories Rail - Editorial Style */}
                <section id="stories-rail" className="scroll-mt-32">
                ...
                {/* 2. Spotlight Banner - Massive & Editorial */}
                {spotlightPoster && (
                    <section className="relative w-full aspect-[4/5] md:aspect-[2/1] overflow-hidden group cursor-pointer rounded-none md:rounded-sm" onClick={() => setSelectedPoster(spotlightPoster)}>
                        ...
                        <div className="absolute inset-0 p-6 md:p-12 flex flex-col justify-between z-10">
                            ...
                            <div className="relative">
                                <h2 className="text-7xl md:text-[8rem] font-display font-black text-white uppercase leading-[0.8] tracking-tighter mb-6 mix-blend-overlay opacity-90">
                                ...
                                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-t border-white/20 pt-6">
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            <OptimizedImage src={spotlightPoster.creator?.avatar} className="w-12 h-12 rounded-full border-2 border-white/20 grayscale group-hover:grayscale-0 transition-all" alt={spotlightPoster.creator?.username || 'unknown'} containerClassName="w-12 h-12 rounded-full" />
                                            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-neon-lime rounded-full border-2 border-black flex items-center justify-center">
                                                <Zap size={8} className="text-black fill-black" />
                                            </div>
                                        </div>
                                        <div>
                                            <p className="text-white font-bold text-lg uppercase tracking-tight">{spotlightPoster.creator?.name || 'Unknown'}</p>
                                            <span className="text-white/60 font-mono text-xs uppercase tracking-widest">@{spotlightPoster.creator?.username || 'unknown'}</span>
                                        </div>
                                    </div>
                ...
                <section>
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-8 border-b border-olive-dark/10 dark:border-white/10 pb-8">
                        <div>
                            <h1 className="text-7xl md:text-8xl font-display font-black text-olive-dark dark:text-cream uppercase tracking-tighter leading-[0.8]">The Feed</h1>
                        </div>
                    </div>
                            ...
                            >
                                ...
                                {CATEGORIES.map(cat => (
                                    <button
                                        ...
                                        className={`shrink-0 snap-start px-5 py-2.5 text-xs font-bold uppercase tracking-wider transition-all border rounded-2xl whitespace-nowrap ${
                                            activeCategory === cat 
